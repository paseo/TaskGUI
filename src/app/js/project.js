    ...
    angular.module('project', ['layout', 'core'])
        .service('projectService', function ($q, VirtualProject, Project, globalParams, fs,$secondView) {
            ...
            this.add = function (obj) {
                ...
                if(!obj.exists) throw new Error(`路径不存在：${file}`);

                function addUnConfiged(){
                ...
                return $q.resolve().then(obj.configed ? addConfiged : addUnConfiged);
            };
        ...
        .directive('projectDrop', function ($q, $notice, projectService) {
            return {
                link: function (scope, element, attrs) {
                    ...
                    function checkFiles(files){
                        return new Promise(function (resolve) {
                            if (files.length != 1) {
                                throw new Error('不支持多选');
                            }

                            var file = files[0];
                            var stat = fs.statSync(file.path);
                            if (!stat.isDirectory()) {
                                throw new Error("不是有效路径");
                            }
                    ...
                    element.on('drop', function (e) {
                        ...
                        $q.when(checkFiles(files)).then(function (dir) {
                            projectService.add(dir);
                        }).catch(function (err) {
                            $notice('error', err.message);
                        });
                    });
